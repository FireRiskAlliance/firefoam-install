...
apt-get install -y git-core build-essential binutils-dev cmake flex bison zlib1g-dev qt4-dev-tools libqt4-dev libqtwebkit-dev gnuplot \
...
apt-get install -y libglu1-mesa-dev libqt4-opengl-dev

# Install additional packages for Scotch/PT-Scotch
apt-get install -y libscotch-dev libptscotch-dev
